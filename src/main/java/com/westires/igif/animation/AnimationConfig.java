...
public record AnimationConfig(
    ...
    public static AnimationConfig load(String animationName, File configFile) {
        ...
        if (fps < 1 || fps > 60) {
            throw new IllegalArgumentException("fps must be 1-60, got: " + fps);
        }
        ...
        return new AnimationConfig(name, source, type, fps, loop,
                fadeIn, stay, fadeOut, maxWidth, maxHeight, fullscreen);
    }

    /**
     * Persists a single key change back to the config.yml file.
     * Supported keys: fps, loop, type, fullscreen, fade-in, stay, fade-out, width, height
     */
    public static void saveKey(File configFile, String key, String value) throws IOException, IllegalArgumentException {
        YamlConfiguration yaml = YamlConfiguration.loadConfiguration(configFile);

        switch (key.toLowerCase()) {
            case "fps" -> {
                int v = parseInt(key, value, 1, 60);
                yaml.set("animation.fps", v);
            }
            ...
            default -> throw new IllegalArgumentException(
                    "Unknown key '" + key + "'. Valid: fps, loop, type, fullscreen, fade-in, stay, fade-out, width, height");
        }
